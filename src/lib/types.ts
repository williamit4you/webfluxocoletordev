...
export type ResponseRule={enabled:boolean;targetPath?:string|null;expectedType?:string|null;emptyBehavior?:string|null;nonEmptyBehavior?:string|null;retryIntervalMinutes?:number|null;maxAttempts?:number|null;failureBehavior?:string|null;description?:string|null;mode?:string|null;operator?:string|null;expectedValue?:string|null;caseSensitive?:boolean|null;onMatchBehavior?:string|null;onMismatchBehavior?:string|null};
export type ResponseRuleEvaluation={enabled:boolean;status:string;action:string;reason:string;targetPath:string;expectedType:string;isEmpty:boolean;attemptCount:number;maxAttempts?:number|null;retryIntervalMinutes?:number|null;nextAttemptAtUtc?:string|null;mode?:string|null;operator?:string|null;actualValue?:string|null;expectedValue?:string|null;matched?:boolean|null};
export type StepApiConfig={url?:string;method?:string;tokenName?:string;scheduleMode?:string;scheduleValue?:string;queryTemplate?:string;validateTls:boolean;sendFieldKeys?:string[];responseMappings?:ResponseFieldMapping[];bodyMappings?:BodyFieldMapping[];scheduleAssist?:StepScheduleAssist|null;headers?:RequestHeader[];bodyTemplate?:string|null;retryOnEmptyArray?:boolean;emptyArrayRetryMinutes?:number|null;emptyArrayAction?:string|null;responseRule?:ResponseRule|null};
export type Step={id?:string;name:string;description?:string;type:number;order:number;assignedUserIds:string[];fields:Field[];apiConfig?:StepApiConfig|null};
export type Flow={id:string;flowKey:string;name:string;description:string;active:boolean;versionNumber:number;lifecycleStatus:string;publishedAt?:string|null;hasDraft:boolean;tokens:FlowToken[];assignedUserIds:string[];steps:Step[]};
export type IntegrationTestResult={success:boolean;statusCode?:number|null;durationMs:number;url:string;method:string;responsePreview?:string|null;errorMessage?:string|null;mappedFields?:Record<string,string>|null;responseRuleEvaluation?:ResponseRuleEvaluation|null};
export type IntegrationAttempt={id:string;triggerType:string;method:string;url:string;responseStatusCode?:number|null;success:boolean;durationMs:number;createdAt:string;requestHeaders?:string|null;requestBody?:string|null;responsePreview?:string|null;errorMessage?:string|null};
